...
describe('Form users', () => {
  ...
  test('Submit form without value', async () => {
    ...
    await dateOfBirthInput.setValue('');
  ...
  test('Set form value', () => {
    ...
    expect(nameInput.setValue(user.name));
    expect(dateOfBirthInput.setValue(user.dateOfBirth));
    expect(emailInput.setValue(user.email));
    expect(phoneInput.setValue(user.phone));
  });
});
